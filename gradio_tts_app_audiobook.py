...
# def switch_model(model_type, progress=gr.Progress()):
#     """Loads the selected model into memory and updates the model_state."""
#     progress(0, desc=f"Unloading current model...")
#     # Unload the old model to free VRAM before loading the new one
#     # In Python, this is handled by garbage collection, but clearing the cache helps
#     if torch.cuda.is_available():
#         torch.cuda.empty_cache()
#
#     progress(0.3, desc=f"Loading {model_type} model...")
...
#
#     progress(1, desc=f"{model_type} model loaded!")
#     return model


# NEW: Function to update the UI when the model type changes
def update_ui_for_model(model_type):
    """Updates the visibility of the language dropdown based on model selection."""
    is_multilingual = "Multilingual" in model_type
    # Return a new Gradio component configuration to update the UI
    return gr.Dropdown(visible=is_multilingual, value="en" if is_multilingual else None)


# MODIFIED: A more robust function to switch models with explicit memory cleanup
def switch_model(model_type, current_model_state, progress=gr.Progress()):
    """Unloads the current model, clears memory, and loads the selected model."""
    ...
    gc.collect()
    if torch.cuda.is_available():
        progress(0.2, desc="Clearing CUDA cache...")
        torch.cuda.empty_cache()

    # Load the new model
    progress(0.4, desc=f"Loading {model_type} model...")
    print(f"Switching to {model_type} model...")
...
def validate_dropdown_voice_assignments(
        text_content, voice_library_path, project_name, voice_counts, character_names,
        char1_voice, char2_voice, char3_voice, char4_voice, char5_voice,
        char6_voice, char7_voice, char8_voice, char9_voice, char10_voice
):
    """
    ...
    # Collect dropdown values (10 total)
    dropdown_values = [
        char1_voice, char2_voice, char3_voice, char4_voice, char5_voice,
        char6_voice, char7_voice, char8_voice, char9_voice, char10_voice
    ]
    ...
    for i, character_name in enumerate(character_names):
        if i < len(dropdown_values):
            ...
            # Not enough dropdowns for all characters
            missing_assignments.append(character_name)
...
with gr.Blocks(css=css, title="Chatterbox TTS - Audiobook Edition") as demo:
        ...
        with gr.TabItem("📖 Audiobook Creation - Single Sample", id="audiobook_single"):
            ...
            with gr.Group():
                ...
                audiobook_output = gr.Audio(
                    ...
                    visible=False,
                    show_download_button=True  # <-- ADD THIS LINE
                )
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Group():
                        ...
                        with gr.Column():
                            char1_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 1",
                                                         visible=False, interactive=True)
                            char2_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 2",
                                                         visible=False, interactive=True)
                            char3_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 3",
                                                         visible=False, interactive=True)
                            char4_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 4",
                                                         visible=False, interactive=True)
                            char5_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 5",
                                                         visible=False, interactive=True)
                            char6_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 6",
                                                         visible=False, interactive=True)
                            char7_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 7",
                                                         visible=False, interactive=True)
                            char8_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 8",
                                                         visible=False, interactive=True)
                            char9_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 9",
                                                         visible=False, interactive=True)
                            char10_dropdown = gr.Dropdown(choices=[("No character found", None)], label="Character 10",
                                                          visible=False, interactive=True)


                    # NEW: Model Selection for Multi-Voice
                    with gr.Group():
                        gr.HTML("<h3>🤖 Model Selection</h3>")

                        multi_model_type = gr.Radio(
                            choices=["English-Only (Legacy)", "Multilingual (New)"],
                            value="English-Only (Legacy)",
                            label="Model Type",
                            info="Choose the TTS model for all characters."
                        )

                        multi_language_id = gr.Dropdown(
                            choices=list(SUPPORTED_LANGUAGES.keys()),
                            value="en",
                            label="Language",
                            info="Select the language for all characters",
                            visible=False  # Hidden by default
                        )
                        gr.HTML(
                            "<p style='font-size:0.8rem;color:gray;'><i>Note: For best results, use a reference voice that matches the selected language to avoid accent transfer.</i></p>")
    ...
    analyze_text_btn.click(
        ...
        outputs=[
            voice_breakdown_display,
            voice_counts_state,
            character_names_state,
            char1_dropdown, char2_dropdown, char3_dropdown, char4_dropdown, char5_dropdown,
            char6_dropdown, char7_dropdown, char8_dropdown, char9_dropdown, char10_dropdown,
            validate_multi_btn,
            multi_audiobook_status
        ]
    )
    ...
    validate_multi_btn.click(
        ...
        inputs=[
            multi_audiobook_text, voice_library_path_state, multi_project_name,
            voice_counts_state, character_names_state,
            char1_dropdown, char2_dropdown, char3_dropdown, char4_dropdown, char5_dropdown,
            char6_dropdown, char7_dropdown, char8_dropdown, char9_dropdown, char10_dropdown
        ],
        outputs=[
